...
int sprawdz(int l, int r, int wart){//maks na przedziale
    int maks = 0
    int lakt = 1;
    int rakt = 1;
    while(l != r){
        if(l % 2 == 0){
            if(lakt == 0 && l + 1 != r){
                maks = max(tree[l + 1].first + wart + tree[l + 1].second, maks);
            }
        }
        if(l % 2 == 1){
            if(lakt == 1){
                maks = max(tree[l].first + wart + tree[l].second, maks);
            }
            lakt = 0;
        }
        ...
        if(r % 2 == 0){
            if(rakt == 1){
                maks = max(tree[r].first + wart + tree[r].second, maks);
            }
            rakt = 0;
        }
        if(r % 2 == 1){
            if(rakt == 0 && r - 1 != l){
                maks = max(tree[r - 1].first + wart + tree[r - 1].first, maks);
            }
        }
...
int main(){
    ...
    for(int i = 0; i < zgloszen; i++){
        ...
        r--;
        /*
        if(sprawdz(l + pocz - 1, r + pocz - 1, ludzi)){
            //cout << l + pocz - 1 << " " << r + pocz - 1;
